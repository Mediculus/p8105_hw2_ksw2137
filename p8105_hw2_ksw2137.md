...
The cleaned `pols_month_data` dataset is a 822 x 9 tibble (column x
row). Source was modified to contain `prez` column that signifies which
political party the presidency was under. The data ranges from Jan 1947
- Jun 2015. They also contain the number of democratic/GOP
...
822 x 11 tibble. Key variables from the 3 datasets:
  ...
  - `month` : Specifies the month of the event
  - `prez`
  - `snp_close`
  - `unemp_count`

Since our reference dataset, `pols_month_data`, ranges from Jan 1947 -
Jun 2015, the `NA` values from `unemployment_data` were not included as
well as `snp_data`’s datapoint on Jul 2015. However, new `NA` values
were formed under `unemployment_data` and `snp_data` column since our
reference ranges from Jan 1947 - Jun 2015.
...
baby_names_data <-
  ...
  mutate(                                                       # Replaces the short-hand with complete description
    ethnicity = str_replace(ethnicity, 
                            "asian and paci(?!fic islander)",   # (?!characters) prevents substitute of the pattern
                            "asian and pacific islander"),      # that contains the specified character
    ethnicity = str_replace(ethnicity,
                            "black non hisp(?!anic)",
                            "black non hispanic"),
    ethnicity = str_replace(ethnicity,
...
baby_names_data %>% 
  ...
  kable(format = "markdown", caption = "test")                                             # Prints a pretty table in markdown
```
...
\*\*\* add comments on why black/white is better; also no aes(size =
count) Finally, for male, white non-hispanic children born in 2016,
