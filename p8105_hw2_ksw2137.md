...
dumpster \#) containing 285 observations each. Identifying variables are
...
column). Source was modified to contain `prez` column that signifies
which political party the presidency was under. The data ranges from Jan
1947 - Jun 2015. They also contain the number of democratic/GOP
governors/senators/representative during the specified dates.
...
Since our reference dataset, `pols_month_data`, ranges from Jan 1947 -
Jun 2015, the `NA` values from `unemployment_data` as well as
`snp_data`’s datapoint on Jul 2015 were not included. However, new
`NA` values were formed under `unemployment_data` and `snp_data` column
since our reference starts from `year` 1947 while `unemployment_data`
...
baby_names_data <-
  ...
  mutate(                                                       # Replaces the short-hand with complete description
    ethnicity = str_replace(ethnicity, 
                            "asian and paci(?!fic islander)",   # (?!characters) prevents substitution of the pattern
                            "asian and pacific islander"),      # that includes the specified character
    ethnicity = str_replace(ethnicity,
                            "black non hisp(?!anic)",
                            "black non hispanic"),
    ethnicity = str_replace(ethnicity,
...
I am not sure what to make of this but will assume unique people for
now…
...
we can see how its ranking fluctuates from 2011-2016.
...
Finally, we produce a scatterplot that shows the difference in numbers
between each of the top 100 white non-hispanic male names.
...
not used as there were too many variables that the color palette
duplicates. Since the names should be unique, there must not be
duplicates. As such, a single color was chosen instead of black to
