...
This difference was apparently due to the different case structure between "Mc __K__ enzie" vs "Mc __k__ enzie". Since these were discnerably considered different in the original data, I decided to keep these as "separate names" despite the same output due to applying `str_to_lower()` to the name variables. 
...
In making this plot, adding color identifiers were considered but was not used as there were too many variables that the color palette duplicates. Since the names should be unique, there must not be duplicates. As such, a single color was chosen instead of black to provide some contrast as well as `alpha =` to somewhat discern stacked datapoints.
