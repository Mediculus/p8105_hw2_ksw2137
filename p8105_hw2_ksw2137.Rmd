...
Since our reference dataset, `pols_month_data`, ranges from Jan 1947 - Jun 2015, the `NA` values from `unemployment_data` were not included as well as `snp_data`'s datapoint on Jul 2015. However, new `NA` values were formed under `unemployment_data` and `snp_data` column since our reference ranges from Jan 1947 - Jun 2015. 
...
*** add comments on why black/white is better; also no aes(size = count)
