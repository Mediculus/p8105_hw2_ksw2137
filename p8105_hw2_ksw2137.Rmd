...
pols_month_data <- 
  ...
  separate(col = mon, into = c("year", "month", "date"), "-") %>%      # Separates y/m/d column into its components
  mutate(month = str_to_lower(month.abb[as.numeric(month)]),           # Change month from "1" to "jan", etc
         prez = prez_gop - prez_dem,                                   # Added column for "presidency" by math (x-y)
...
snp_data <- 
  ...
  janitor::clean_names() %>%                                        # Cleans the column names
  separate(col = date, into = c("month", "date", "year"), "/") %>%  # Separates y/m/d column into its components
  mutate(month = as.numeric(month)) %>%                             # Turns the vector into numeric
...
* Case structure of the names changed between 2012 - 2013 from "UPPERCASE" to "Name"
